...
- **Board-spezifische Blöcke ausblenden** – Ein Profil kann `hideCategories` / `hideSubCategories` / `hideBlockIds` deklarieren (siehe `lolin_s2_mini`). `block_builder.js` (`buildToolboxCategories`) und `toolbox.js` (`buildFinalToolbox`, Matrix) filtern die Toolbox danach. Der S2 Mini zeigt so nur generische GPIO-/Analog-Blöcke + externen NeoPixel; Motor/Servo/Ton/Onboard-RGB/LCD/TM1637/Matrix/Batterie sind verborgen.
