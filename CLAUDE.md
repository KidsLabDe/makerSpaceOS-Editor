...
- **`grove_rgb_lcd.py`** (LCD-Blöcke) liegt unter `lib/` im Repo und muss nach `CIRCUITPY/lib/` kopiert werden. Text-Controller (JHD1313/JHD1802) immer @ 0x3E; die **RGB-Beleuchtung wird automatisch erkannt** (erst 0x30/SGM31323 = V5, dann 0x62/PCA9633 = V4). **Grove-16x2-LCD (Mono-Versionen)** haben **keinen** I2C-Beleuchtungstriever → Library fällt auf Text-Modus zurück (keine `OSError`, `set_rgb()` no-op, Hinweis im Serial-Monitor). Generator erzeugt `GroveRgbLcd(_i2c_lcd)` ohne `rgb_addr`.
- **Drehgeber braucht benachbarte Pins**: `rotaryio.IncrementalEncoder` (PIO) verlangt zwei **adjazente** GPIOs (GPn + GPn±1, gleiche Pin-Gruppe) – sonst `RuntimeError: Pins must be sequential GPIO pins` (de-Build: „Pins müssen geordnete GPIO-Pins sein") beim Start. Deshalb nutzen Encoder-Blöcke (`sensor_encoder`, `when_encoder`) das `groveRole: 2pin_sequential`-Dropdown (boards.js filtert auf Ports mit Nachbar-Pinpaar; MAKER-PI: Grove 1, 2, 3, 4, 6 – **nicht** 5 = GP6/GP26 und 7 = GP7/GP28). Die Generator setzen bei ungültigem Port (alte Projekte) eine `setWarningText`.
...
- **adafruit_bmp280** muss manuell auf `CIRCUITPY/lib/` kopiert werden. Alle anderen (`adafruit_dht`, `neopixel`, `adafruit_motor`) sind Standard im Adafruit CircuitPython Bundle. Der Ultraschall-Block hat ein **TYPE-Dropdown** (`fieldType: sensor_type_dropdown`): Default `grove` = Grove-Ultraschall-Ranger (Single-Pin, `lib/grove_ultrasonic.py`); `sr04` = freier HC-SR04 (2-Pin: TRIG = Signal-Pin, ECHO = 2. Pin `pin1` des Grove-Ports, `lib/sr04.py` → beide nach `CIRCUITPY/lib/` kopieren). Generatoren: `_sonarExpression()` in `generator.js`. **Wichtig:** „Ausführen" läuft ohne Soft-Reboot – `serial.js` (`PIN_RESET_PRELUDE`) ruft daher vor jedem Lauf `deinit()`/`exit()` auf alle Objekte des vorherigen Laufs. Wrapper-Klassen in `lib/`, die Pins dauerhaft halten (z. B. `SR04`), **müssen `deinit()` implementieren**, sonst bleiben die Pins belegt → `ValueError: GPx in Benutzung`. Neue/veränderte `lib/`-Dateien braucht das Board erst nach USB-Trennung (Relay), weil `sys.modules` im REPL-Session cached bleibt. **Play-Handshake** (`uploadAndRun`): Nach jedem Play steht das Board in der **Raw-REPL** (nicht am `>>>`-Prompt) – deshalb normalisiert der Handshake zuerst den Zustand (Ctrl+C + Enter bis `>>>` erscheint, notfalls Ctrl+B zum Verlassen der Raw-REPL, max. 3 Durchgänge), prüft die `OK`-Kompilierbestätigung nach dem Start und sperrt parallele Starts (Doppel-Play).
...
- **`scripts/sync_lib.sh`**: kopiert `lib/` auf das Board. Findet das CIRCUITPY-Volume automatisch (macOS `/Volumes/`, Linux `/run/media/` + `/media/`); `SEARCH_ROOTS`-Env var überschreibt die Suchpfade. Ohne `rsync` fällt auf `cp` zurück. Danach immer USB neu stecken (sys.modules-Cache).
- **`scripts/pull_firmware.sh`**: zieht mit `picotool save -a` den kompletten Flash eines Boards im **BOOTSEL-Modus** (MAKER-PI: doppelter Reset beim USB-Anstecken) nach `firmware/makerSpaceOS_firmware_rp2040_<Zeitstempel>.uf2`.
