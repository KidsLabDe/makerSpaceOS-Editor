...
Detailed guide with photos: \`docs/esp32-d1-r32.md\` in the project.`,
  ...
  const GROVE_ROLE_LABELS = {
    digital: L('digitaler Grove-Port', 'digital Grove port'),
    analog:  L('analoger Grove-Port', 'analog Grove port'),
    i2c:     L('I2C-Port', 'I2C port'),
    '2pin':  L('Grove-Port (beide Pins)', 'Grove port (both pins)'),
  };
  ...
  function buildEntries() {
    ...
    for (const id of boardIds) {
      ...
      _entries.set(key, {
        ...
        label: p.name + (id === BOARD_ID ? L('  · aktiv', '  · active') : ''),
        colour: '#64748B',
        tooltip: '',
        doc: (IS_EN ? BOARD_DOCS_EN[id] : BOARD_DOCS[id]) || BOARD_DOCS[id] || '',
        baseDir: '',
      });
      boardGroup.keys.push(key);
    }
    _sections.push({ id: 'Boards', label: 'Boards', colour: '#64748B', groups: [boardGroup] });
    ...
    const addCoreEntry = (c, group) => {
      ...
      _entries.set(key, {
        ...
        label: LF(c, 'label'), colour: c.colour, section: c.section,
        tooltip: LF(c, 'tooltip'), doc: LF(c, 'doc'), baseDir: '',
      });
      group.keys.push(key);
    };
    ...
    for (const sec of CORE_SECTIONS) {
      ...
      if (group.keys.length) {
        _sections.push({ id: sec.name, label: (IS_EN && sec.label_en) || sec.name, colour: sec.colour, groups: [group] });
      }
    }
    ...
    for (const catDef of BLOCKS_CATALOG.categories) {
      ...
      const addBlocks = (sub, defs) => {
        ...
        for (const def of defs) {
          ...
          _entries.set(key, {
            ...
            label: LF(def, 'label') || def.id,
            colour: def.colour || catDef.colour,
            tooltip: LF(def, 'tooltip'),
            doc: LF(def, 'doc'),
            baseDir: dir,
          });
          g.keys.push(key);
      ...
      const subLabel = sc => (IS_EN && catDef.subCategories_en && catDef.subCategories_en[sc]) || sc;
      if (catDef.subCategories && catDef.subCategories.length) {
        for (const sc of catDef.subCategories) {
          if (hideSubs.has(sc)) continue;
          addBlocks(subLabel(sc), catBlocks.filter(b => b.subCategory === sc));
        }
      ...
      if (groups.length) _sections.push({ id: catDef.id, label: LF(catDef, 'label'), colour: catDef.colour, groups });
    }
    ...
    if (matrixGroup.keys.length) {
      const lichter = _sections.find(s => s.id === 'Lichter');
      if (lichter) lichter.groups.push(matrixGroup);
      else _sections.push({ id: 'Lichter', label: L('Lichter', 'Lights'), colour: MATRIX_COLOUR, groups: [matrixGroup] });
    }
  }
  ...
  function buildDom() {
    ...
    _root.innerHTML = `
      <div id="library-header">
        <span id="library-title">${L('📚 Bauteil-Bibliothek', '📚 Component Library')}</span>
        <input id="library-search" type="search" placeholder="${L('Suchen… (z.B. Taster, LED, Abstand)', 'Search… (e.g. button, LED, distance)')}" autocomplete="off">
        <span id="library-count"></span>
        <button id="library-close" class="btn btn-ghost" title="${L('Bibliothek schließen (Esc)', 'Close library (Esc)')}">
      ...
      <div id="library-main">
        ...
        <div id="library-detail">
          <div id="library-placeholder">
            ...
            <p>${L('Wähle links ein Bauteil oder einen Block aus –<br>hier erscheint dann die Erklärung dazu.', 'Choose a component or block on the left –<br>its explanation will appear here.')}</p>
          </div>
        </div>
      </div>`;
  ...
  function renderTree(filter) {
    ...
    _root.querySelector('#library-count').textContent =
      q ? `${total} ${L('Treffer', 'matches')}` : `${total} ${L('Einträge', 'entries')}`;
  }
  ...
  function cardHtml(entry) {
    ...
    if (!entry.tooltip && !entry.doc) {
      parts.push('<p class="lib-intro">' + L('Für diesen Eintrag gibt es noch keine ausführliche Erklärung.', 'There is no detailed explanation for this entry yet.') + '</p>');
    }
  ...
  function cardSubtitle(entry) {
    if (entry.kind === 'board') return '<div class="lib-card-sub">' + L('Board-Profil', 'Board profile') + '</div>';
    const def = entry.def;
  ...
  function cardChips(entry) {
    ...
    if (def) {
      if (def.blockCategory) {
        const catDef = (BLOCKS_CATALOG.categories || []).find(c => c.id === def.blockCategory);
        const catLbl = catDef ? LF(catDef, 'label') : def.blockCategory;
        const subLbl = def.subCategory
          ? ((IS_EN && catDef && catDef.subCategories_en && catDef.subCategories_en[def.subCategory]) || def.subCategory)
          : '';
      ...
      if (def.hardware && def.hardware.kitStandard) chips.push(L('✓ im Kit enthalten', '✓ included in the kit'));
    } else if (entry.kind === 'core') {
      const secDef = CORE_SECTIONS.find(sc => sc.name === entry.section);
      const secLbl = secDef ? ((IS_EN && secDef.label_en) || secDef.name) : entry.section;
      chips.push(escHtml(entry.section === 'Lichter' ? L('Lichter · 8×8 Matrix', 'Lights · 8×8 matrix') : (secLbl || L('Grundlagen', 'Basics'))));
    }
  ...
  function boardPortTable(boardId) {
    ...
    return `<h3>${L('Anschlüsse (Ports)', 'Connectors (ports)')}</h3>
      <table class="lib-table">
        <thead><tr><th>Port</th><th>${L('Pins (Signal + zweiter Pin)', 'Pins (signal + second pin)')}</th><th>${L('Kann', 'Can do')}</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>`;
  }
  ...
  function buildDrawer() {
    ...
    _drawer.innerHTML = `
      ...
      <div id="docs-panel-footer">
        <button id="docs-panel-library" class="btn btn-ghost">${L('📚 In der Bibliothek öffnen', '📚 Open in the library')}</button>
      </div>`;
